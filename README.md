> [!WARNING]  
> Use at your own risk. Oris Meet is the open source community edition of Cal.com and it is intended for users who want to self-host their own Oris Meet instance. It is strictly recommended for personal, non-production use. Please review all installation and configuration steps carefully. Self-hosting requires advanced knowledge of server administration, database management, and securing sensitive data. Proceed only if you are comfortable with these responsibilities.

> [!TIP]
> For any commercial and enterprise-ready scheduling infrastructure, use Cal.com, not Oris Meet; hosted by us or get invited to on-prem enterprise access here: <a href="https://cal.com/sales">https://cal.com/sales</a>
...
**Oris Meet** is the community-driven, fully open-source scheduling platform — a fork of [Cal.com](https://cal.com) with all enterprise/commercial code removed.

Oris Meet is **100% MIT-licensed** with no proprietary "Enterprise Edition" features. It's designed for individuals and self-hosters who want full control over their scheduling infrastructure without any commercial dependencies.
...
> **Note:** Oris Meet is a self-hosted project. There is no hosted/managed version. You run it on your own infrastructure.
...
Here’s what you need to run Oris Meet.
...
#### Running Oris Meet with Docker Compose
...
5. Start Oris Meet via docker compose

   To run the complete stack, which includes a local Postgres database, Oris Meet web app, and Prisma Studio:

   ```bash
   docker compose up -d
   ```

   To run Oris Meet web app and Prisma Studio against a remote database, ensure that DATABASE_URL is configured for an available database and run:

   ```bash
   docker compose up -d calcom studio
   ```

   To run only the Oris Meet web app, ensure that DATABASE_URL is configured for an available database and run:
...
6. Open a browser to [http://localhost:3000](http://localhost:3000), or your defined NEXT_PUBLIC_WEBAPP_URL. The first time you run Oris Meet, a setup wizard will initialize. Define your first user, and you're ready to go!
...
4. Re-start the Oris Meet stack
...
4. Build the Oris Meet docker image:
...
5. Build Oris Meet via docker compose (DOCKER_BUILDKIT=0 must be provided to allow a network bridge to be used at build time. This requirement will be removed in the future)

   ```bash
   DOCKER_BUILDKIT=0 docker compose build calcom
   ```

6. Start Oris Meet via docker compose
...
7. Open a browser to [http://localhost:3000](http://localhost:3000), or your defined NEXT_PUBLIC_WEBAPP_URL. The first time you run Oris Meet, a setup wizard will initialize. Define your first user, and you're ready to go!
...
| NEXTAUTH_URL | Location of the auth server. By default, this is the Oris Meet docker instance itself. | optional | `{NEXT_PUBLIC_WEBAPP_URL}/api/auth` |
...
| CALCOM_TELEMETRY_DISABLED | Allow Oris Meet to collect anonymous usage data (set to `1` to disable) | optional | |
...
You can deploy Oris Meet on [Railway](https://railway.app). The team at Railway also have a [detailed blog post](https://blog.railway.app/p/calendso) on deploying on their platform.
...
You can deploy Oris Meet on [Northflank](https://northflank.com). The team at Northflank also have a [detailed blog post](https://northflank.com/guides/deploy-calcom-with-northflank) on deploying on their platform.
...
Oris Meet is fully open source, licensed under the [MIT License](https://opensource.org/license/mit).

Unlike Cal.com's "Open Core" model, Oris Meet has **no commercial/enterprise code**. The entire codebase is available under the same open-source license.
...
9. Under Authorized redirect URI's, select Add URI and then add the URI `<Oris Meet URL>/api/integrations/googlecalendar/callback` and `<Oris Meet URL>/api/auth/callback/google` replacing Oris Meet URL with the URI at which your application runs.
10. The key will be created and you will be redirected back to the Credentials page. Select the newly generated client ID under OAuth 2.0 Client IDs.
11. Select Download JSON. Copy the contents of this file and paste the entire JSON string in the `.env` file as the value for `GOOGLE_API_CREDENTIALS` key.

#### _Adding google calendar to Oris Meet App Store_
...
1. Add extra redirect URL `<Oris Meet URL>/api/auth/callback/google`
...
4. Set the **Web** redirect URI to `<Oris Meet URL>/api/integrations/office365calendar/callback` replacing Oris Meet URL with the URI at which your application runs.
...
8. Set the "OAuth Redirect URL" under "OAuth Information" as `<Oris Meet URL>/api/integrations/zoomvideo/callback` replacing Oris Meet URL with the URI at which your application runs.
...
11. Click "Done".
12. You're good to go. Now you can easily add your Zoom integration in the Oris Meet settings.
...
5. Set the Redirect URL for OAuth `<Oris Meet URL>/api/integrations/basecamp3/callback` replacing Oris Meet URL with the URI at which your application runs.
...
7. Set the Redirect URL for OAuth `<Oris Meet URL>/api/integrations/hubspot/callback` replacing Oris Meet URL with the URI at which your application runs.
...
10. You're good to go. Now you can see any booking in Oris Meet created as a meeting in HubSpot for your contacts.
...
7. Set the Redirect URL for OAuth `<Oris Meet URL>/api/integrations/zohocrm/callback` replacing Oris Meet URL with the URI at which your application runs.
...
10. You're good to go. Now you can easily add your ZohoCRM integration in the Oris Meet settings.
...
Oris Meet uses [Unkey](https://unkey.com) for rate limiting. This is an optional feature and is not required for self-hosting.
...
Note: If you don't configure Unkey, Oris Meet will work normally without rate limiting enabled.

## Contributing

We welcome contributions! Whether it's fixing a typo, improving documentation, or building new features, your help makes Oris Meet better.

> **Important:** Oris Meet is a community fork. Contributions to this repo do **not** flow to Cal.com's production platform. See [CONTRIBUTING.md](./CONTRIBUTING.md) for details.
...
Don't code but still want to contribute? Join our [Discussions](https://github.com/calcom/cal.diy/discussions) and help translate Oris Meet into your language.

<!-- ACKNOWLEDGEMENTS -->

## Acknowledgements

Oris Meet is built on the foundation created by [Cal.com](https://cal.com) and the many contributors to the original project. Special thanks to:
